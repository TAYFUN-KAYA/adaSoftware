...
window.onload = function () {
    var new_ip;
    fetch("https://api.ipify.org/?format=json").then(result => result.json()).then(
        data => new_ip = data.ip
    );
    console.log("İp Adresi alındı.");
    console.log("İp Adress : ", new_ip);
    userAction();
}
...
function handleClickPayForms() {
    $("#paynext").on("click", function (event) {
        ...
        if (name.length < 3 || surname.length < 2 || identitynumber.length < 5 || inputAddress.length < 5 || inputCity.length < 3 || country.length < 3 || inputZip.length < 3 || phone.length < 5) {
            alert(mesaj_other);
        } else if (checkPage_status == false) {
            alert(mesaj_check)
        } else {
            if (check) {
                $.ajax({
                    ...
                    method: "POST",
                    data: { name: name, email: email, surname: surname, identitynumber: identitynumber, inputAddress: inputAddress, inputAddress2: inputAddress2, country: country, inputCity: inputCity, inputZip: inputZip, phone: phone, packageStatus: packageStatus, ip_adress: give_ip, price: price,dolar:dolar,mont_year:mont_year,mont_year_other:mont_year_other },
